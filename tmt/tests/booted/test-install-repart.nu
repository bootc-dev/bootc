...
"
    ...
    run_install_to_disk localhost/bootc-repart-noroot ["--filesystem" "ext4" "--root-size" "5G"]
    ...
    try {
        ...
        # Verify root is not using all disk space (--root-size 5G was specified)
        let root_part = ($children | last)
        let root_size_bytes = ($root_part.size | into int)
        let five_gb = (5 * 1024 * 1024 * 1024)
        assert ($root_size_bytes <= $five_gb) $"Root partition should be ~5G, got ($root_part.size)"
