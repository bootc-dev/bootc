...
use crate::composefs_consts::{
    BLS_ENTRY_PREFIX, COMPOSEFS_STAGED_DEPLOYMENT_FNAME, COMPOSEFS_TRANSIENT_STATE_DIR,
    TYPE1_ENT_PATH, TYPE1_ENT_PATH_STAGED,
};
...
fn rollback_composefs_entries(host: &Host, boot_dir: &Dir, bootloader: Bootloader) -> Result<()> {
    ...
    for entry in original_entries.entries_utf8()? {
        ...
        if entry.file_type()?.is_file() && !entry_name.starts_with(BLS_ENTRY_PREFIX) {
            original_entries
                .copy(entry_name.clone(), &rollback_entries_dir, entry_name)
