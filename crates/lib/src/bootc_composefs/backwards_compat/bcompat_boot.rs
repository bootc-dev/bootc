...
use crate::{
    ...
    composefs_consts::{
        BLS_ENTRY_PREFIX, ORIGIN_KEY_BOOT, ORIGIN_KEY_BOOT_TYPE, STATE_DIR_RELATIVE,
        TYPE1_BOOT_DIR_PREFIX, TYPE1_ENT_PATH, TYPE1_ENT_PATH_STAGED, UKI_NAME_PREFIX,
        USER_CFG_STAGED,
    },
...
fn create_staged_bls_entries(boot_dir: &Dir, entries: &Vec<(String, BLSConfig)>) -> Result<()> {
    ...
    for entry in original_entries.entries_utf8()? {
        ...
        if entry.file_type()?.is_file() && !entry_name.starts_with(BLS_ENTRY_PREFIX) {
            original_entries
                .copy(entry_name.clone(), &staged_entries, entry_name)
