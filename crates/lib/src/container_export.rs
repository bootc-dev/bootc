...
fn handle_kernel_relocation<W: Write>(
    ...
    let (vmlinuz_path, initramfs_path) = match &kernel_info.path {
        crate::kernel::KernelPath::Vmlinuz { path, initramfs } => (Some(path), Some(initramfs)),
        crate::kernel::KernelPath::Uki(_) => (None, None),
    };

    if let Some(vmlinuz_path) = vmlinuz_path {
