...
fn wait_for_vm_ready(sh: &Shell, vm_name: &str) -> Result<(u16, String)> {
    ...
    for attempt in 1..=VM_READY_TIMEOUT_SECS {
        if let Ok(json_output) = cmd!(sh, "bcvk libvirt inspect {vm_name} --format=json")
            .ignore_stderr()
            .read()
            && let Ok(json) = serde_json::from_str::<serde_json::Value>(&json_output)
                && let (Some(ssh_port), Some(ssh_key)) = (
...
pub(crate) fn run_tmt(sh: &Shell, args: &RunTmtArgs) -> Result<()> {
    ...
    let context = args
        ...
        .chain(std::iter::once("--context=running_env=image_mode".to_string()))
    ...
    if variant_id == "coreos" {
        ...
        .run()
        .with_context(|| "Saving container image to tar".to_string())?;
    }
...
pub(crate) fn update_integration() -> Result<()> {
    ...
    for test in &tests {
        ...
        if let serde_yaml::Value::Mapping(map) = &test.tmt
            && let Some(summary) = map.get(&serde_yaml::Value::String(FIELD_SUMMARY.to_string())) {
                plan_value.insert(
        ...
        if let serde_yaml::Value::Mapping(map) = &test.tmt
            && let Some(adjust) = map.get(&serde_yaml::Value::String(FIELD_ADJUST.to_string())) {
                plan_value.insert(
